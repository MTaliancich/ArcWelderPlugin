...
def dict_encode(d):
    ...
    def dict_key_value_encode(s):
        ...
        try:
            if isinstance(s, str):
                return s.encode(errors='ignore', encoding='utf-8')
...
def search_string(string_to_search, strings_to_find, start_index=0):
    ...
    for item in strings_to_find:
        # skip spaces
        if index > search_strlen - 1:
            return -1
        item_len = len(item)
        item_max_index = search_strlen - item_len
        while index <= item_max_index and string_to_search[index] == " ":
            index += 1

        if index > search_strlen - item_len:
            # We ran out of string to search
            return -1

        # Extract the possible match
        string_part = string_to_search[index: index + item_len].upper()
        if string_part == item:
            index += item_len
        else:
            return -1
...
def parse_datetime(datetime_string):
    date_formats = ["%b %d %Y %H:%M:%S", "%b %d %Y %H:%M:%S", ]
...
def is_version_in_versions(current_version_string, version_checks, compare_type="semantic"):
    ...
    for version_check in [x.strip() for x in version_checks.split(",")]:
        ...
        if compare_type == "date":
            ...
            logger.error(
                "Unknown compare type '%s' for the version check '%s'.", compare_type, version_check
            )
